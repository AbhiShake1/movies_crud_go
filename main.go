...
func getMovieRoutes(req *gin.Engine) {
	...
	req.PUT(fmt.Sprintf("%v:id", MOVIE), func(c *gin.Context) {
		id := c.Param("id")
		views.UpdateMovie(c, id)
	})
	req.DELETE(fmt.Sprintf("%v:id", MOVIE), func(c *gin.Context) {
		id := c.Param("id")
		views.DeleteMovie(c, id)
	})
	req.GET(fmt.Sprintf("%v:id", MOVIE), func(c *gin.Context) {
		id := c.Param("id")
		views.GetMovie(c, id)
	})
}
